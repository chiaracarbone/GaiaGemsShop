export const stonesList = [
    {
        id: 1,
        name: "Labradorite",
        price: "5€/piece",
        image: 'img/stones001.jpg',
    },
    {
        id: 2,
        name: "Chryscolla",
        price: "2€/piece",
        image: 'img/stones002.jpg',
    },
    {
        id: 3,
        name: "Turquoise",
        price: "2.50€/piece",
        image: 'img/stones003.jpg',
    },
    {
        id: 4,
        name: "Amethyst",
        price: "2€/piece",
        image: 'img/stones004.jpg',
    },
    {
        id: 5,
        name: "Malachite",
        price: "3€/piece",
        image: 'img/stones005.jpg',
    },
    {
        id: 6,
        name: "Humatite",
        price: "3.50€/piece",
        image: 'img/stones006.jpg',
    }
];

export const detailsStones = [
    {
        id: 1,
        name: "Labradorite",
        price: "5€/piece",
        image: 'img/stones001.jpg',
        info:`Lorem ipsum dolor sit amet, per ad tale at partem accusata petentium, nibh abhorreant no mea. Reque etiam instructior eam an. Vis vide wisi docendi ea, velit putent convenire te eam.
        Sed ne elitr intellegat, falli inimicus suavitate ex eos. 
        Id est habeo ludus disputationi, adolescens Sumo dolore volumus eu nec, debet aliquip expetendis vix et    
       Dicant assentior usu te. Id graeco omnium mel. Ne oratio appareat persecuti vim. Elit bonorum deleniti ex has. Qui fugit scripserit in, has ea dolorem evertitur. Timeam malorum pertinacia eos an. `,
    },
    {
        id: 2,
        name: "Chryscolla",
        price: "2€/piece",
        image: 'img/stones002.jpg',
        info:`Lorem ipsum dolor sit amet, per ad tale at partem accusata petentium, nibh abhorreant no mea. Reque etiam instructior eam an. Vis vide wisi docendi ea, velit putent convenire te eam.
        Sed ne elitr intellegat, falli inimicus suavitate ex eos. 
        Id est habeo ludus disputationi, adolescens Sumo dolore volumus eu nec, debet aliquip expetendis vix et    
       Dicant assentior usu te. Id graeco omnium mel. Ne oratio appareat persecuti vim. Elit bonorum deleniti ex has. Qui fugit scripserit in, has ea dolorem evertitur. Timeam malorum pertinacia eos an. `,
    },
    {
        id: 3,
        name: "Turquoise",
        price: "2.50€/piece",
        image: 'img/stones003.jpg',
        info:`Lorem ipsum dolor sit amet, per ad tale at partem accusata petentium, nibh abhorreant no mea. Reque etiam instructior eam an. Vis vide wisi docendi ea, velit putent convenire te eam.
        Sed ne elitr intellegat, falli inimicus suavitate ex eos. 
        Id est habeo ludus disputationi, adolescens Sumo dolore volumus eu nec, debet aliquip expetendis vix et    
       Dicant assentior usu te. Id graeco omnium mel. Ne oratio appareat persecuti vim. Elit bonorum deleniti ex has. Qui fugit scripserit in, has ea dolorem evertitur. Timeam malorum pertinacia eos an. `,
    },
    {
        id: 4,
        name: "Amethyst",
        price: "2€/piece",
        image: 'img/stones004.jpg',
        info:`Lorem ipsum dolor sit amet, per ad tale at partem accusata petentium, nibh abhorreant no mea. Reque etiam instructior eam an. Vis vide wisi docendi ea, velit putent convenire te eam.
        Sed ne elitr intellegat, falli inimicus suavitate ex eos. 
        Id est habeo ludus disputationi, adolescens Sumo dolore volumus eu nec, debet aliquip expetendis vix et    
       Dicant assentior usu te. Id graeco omnium mel. Ne oratio appareat persecuti vim. Elit bonorum deleniti ex has. Qui fugit scripserit in, has ea dolorem evertitur. Timeam malorum pertinacia eos an. `,
    },
    {
        id: 5,
        name: "Malachite",
        price: "3€/piece",
        image: 'img/stones005.jpg',
        info:`Lorem ipsum dolor sit amet, per ad tale at partem accusata petentium, nibh abhorreant no mea. Reque etiam instructior eam an. Vis vide wisi docendi ea, velit putent convenire te eam.
        Sed ne elitr intellegat, falli inimicus suavitate ex eos. 
        Id est habeo ludus disputationi, adolescens Sumo dolore volumus eu nec, debet aliquip expetendis vix et    
       Dicant assentior usu te. Id graeco omnium mel. Ne oratio appareat persecuti vim. Elit bonorum deleniti ex has. Qui fugit scripserit in, has ea dolorem evertitur. Timeam malorum pertinacia eos an. `,
    },
    {
        id: 6,
        name: "Humatite",
        price: "3.50€/piece",
        image: 'img/stones006.jpg',
        info:`Lorem ipsum dolor sit amet, per ad tale at partem accusata petentium, nibh abhorreant no mea. Reque etiam instructior eam an. Vis vide wisi docendi ea, velit putent convenire te eam.
        Sed ne elitr intellegat, falli inimicus suavitate ex eos. 
        Id est habeo ludus disputationi, adolescens Sumo dolore volumus eu nec, debet aliquip expetendis vix et    
       Dicant assentior usu te. Id graeco omnium mel. Ne oratio appareat persecuti vim. Elit bonorum deleniti ex has. Qui fugit scripserit in, has ea dolorem evertitur. Timeam malorum pertinacia eos an. `,
    }
];